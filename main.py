...
class Rag:
    def __init__(self, pre_process_impl: PreProcessDataInterface, index_data_impl: IndexerInferface, get_final_answers_impl: GetFinalAnswersInterface):
    ...
    def answer_queries(self, queries):
        web_text_units = self.pre_process_impl.pre_proccess_data()
...
def run_rag():
    ...
    pre_process_impl = WebDataPreProccessor()
    index_data_impl = Bm25Indexer()
    get_final_answers_impl = GetFinalAnswersImplementation()

    rag = Rag(pre_process_impl, index_data_impl, get_final_answers_impl)
