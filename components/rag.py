...
class Rag:
    ...
    def retrieve_from_all_indexers(self, queries: List[Query], k: int):
        ...
        for query in queries:
            answer_sources = query.answer_sources
            for i in range(1, len(answer_sources)//2):
                answer_sources.insert(i*2, answer_sources[len(answer_sources)//2 + i])
            query.answer_sources = answer_sources[:len(answer_sources)//2]
